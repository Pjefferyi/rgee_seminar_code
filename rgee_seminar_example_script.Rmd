...
Before I start Using rgee, I need to authorize it to manage Earth Engine resources, Google Drive, and Google Cloud Storage resources through my Google account. 

This process should be a bit simpler than what is shown below.It seems rgee might need an update. 
...
Next we want to load our spatial datasets. Fortemperature we will use the MOD11A1.061 Terra Land Surface Temperature and Emissivity Daily Global 1km.
...
For the city of Guelph, we will use a polygon dataset available here: 
https://www.arcgis.com/apps/mapviewer/index.html?layers=6edf63b03c95423bae1ab9c590866838 

This is actually a set of polygons for each property in Guelph, but I will merge those into a single polygon. As you can tell from looking at the output, there are some holes in the output, but it will do for our purposes. 

We will use the guelph city polygon as a "mask" to filter through our dataset of land surface tempefature (LST). We only need data for the city of Guelph. 
...
# We use filter our MODIS feature collection and extract the mean temperature in 2023
...
It is a bit hard to make sense of our image here, especially since their is no scale bar. 

We can convert to image to a file and export it to Google drive (In this case I save it in my own account).

Note that when we plot the data now, we are no longer using functions from Earth Engine. 
...
recall that we are working with an image collection. We'll iterate through every month and year, and for each month, we will filter our collection to get all image for the month. Then, we will extract the mean temperature for the month by reducing this subset of the collection. We will store our monthly mean data, as well as year and month in a feature collection.  

This process is a but similar to a nested loop in R. We loop through each year, and for each month in a given year. 

```{r, echo=T, message=F}

# The years and month that we will be ierating through 
